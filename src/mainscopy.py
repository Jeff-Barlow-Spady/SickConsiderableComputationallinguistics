...
# MongoDB setup
MONGO_DETAILS = "mongodb+srv://jeffbarlowspady:f5nlDfzrROpHOcAg@longtrees.j3weklg.mongodb.net/?retryWrites=true&w=majority&appName=LongTrees"
...
tree_list_collection = database.get_collection("Tree")
...
async def get_seed_sources(request: Request):
    try:
        seed_sources = await SeedSource.find()
        return templates.TemplateResponse(
            "list_seed_sources.html", {"request": request, "seed_sources": seed_sources_collection}
        )
    except ServiceException as e:
        handle_service_exception(e)


@app.post("/seed_sources/add/", response_class=HTMLResponse())
async def create_seed_source(
    ...
    quantity: int = Form(...),
    scarification_instructions: Optional[str] = Form(None),
    stratification_instructions: Optional[str] = Form(None),
    date_added: date = Form(...),
    seeds_issued: int = Form(...),
    geographic_location: Optional[str] = Form(None),
    supplier: Optional[str] = Form(None),
    viability_duration: Optional[str] = Form(None),
):
    try:
        if not succession_number:
            raise HTTPException(status_code=400, detail="Succession number is required")
        date_added_obj = date.fromisoformat(date_added)
        if not date_added_obj:
        ...
        return templates.TemplateResponse(
            "_seed_source_item.html", {"request": request, "seed_source": seed_source}
        )
    except ServiceException as e:
...
@app.get("/seed_sources/{_id}", response_class=HTMLResponse)
...
@app.post("/seed_sources/{_id}/edit", response_class=HTMLResponse)
...
@app.post("/seed_sources/{_id}/delete", response_class=HTMLResponse)
async def delete_seed_source(request: Request, id: str):
    try:
        seed_source = await SeedSource.get(id)
        await seed_source.delete()
        return templates.TemplateResponse(
            "list_seed_sources.html", {"request": request}
        )
    except ServiceException as e:
        handle_service_exception(e)
...
@app.post("/growers/add", response_class=HTMLResponse)
async def create_grower(
    ...
    try:
        ...
        await grower.insert()
        return templates.TemplateResponse(
            "_grower_item.html", {"request": request, "grower": grower}
        )
    except ServiceException as e:
        handle_service_exception(e)


@app.get("/growers/{_id}", response_class=HTMLResponse)
...
@app.post("/growers/{_id}/edit", response_class=HTMLResponse)
...
@app.post("/growers/{_id}/delete", response_class=HTMLResponse)
...
async def get_sub_successions(request: Request):
    try:
        sub_successions = await SubSuccession.find()
        return templates.TemplateResponse(
...
@app.post("/sub_successions/add", response_class=HTMLResponse)
...
@app.get("/sub_successions/{_id}", response_class=HTMLResponse)
...
@app.post("/sub_successions/{_id}/edit", response_class=HTMLResponse)
...
@app.post("/sub_successions/{_id}/delete", response_class=HTMLResponse)
async def delete_sub_succession(request: Request, id: str):
    try:
        sub_succession = await SubSuccession.get(id)
        await sub_succession.delete()
        return templates.TemplateResponse(
            "list_sub_successions.html", {"request": request}
        )
    except ServiceException as e:
        handle_service_exception(e)
...
@app.post("/trees/add", response_class=HTMLResponse)
async def create_tree(
    ...
    try:
        ...
        await tree.insert()
        return templates.TemplateResponse(
            "_tree_item.html", {"request": request, "tree": tree}
        )
    except ServiceException as e:
        handle_service_exception(e)


@app.get("/trees/{_id}", response_class=HTMLResponse)
...
@app.post("/trees/{_id}/edit", response_class=HTMLResponse)
...
@app.post("/trees/{_id}/delete", response_class=HTMLResponse)
